...
<?php
	...
	$start = ($page - 1) * $limit;
	$result = $conn->query("SELECT * FROM users WHERE admin_verification_status = 0 ORDER BY id DESC LIMIT $start, $limit");
	$users = $result->fetch_all(MYSQLI_ASSOC);

	$result1 = $conn->query("SELECT count(id) AS id FROM users WHERE admin_verification_status = 0");
